...
public: // getter
    ...
    tg::isize3 getDimensions() const override { return { mWidth, 1, 1 }; }
    
    Shape getShape() const { return { mInternalFormat, mWidth, mImmutableMipmaps }; }
...
public:
    ...
    struct BoundTexture1D
    {
        ...
        void setData(GLenum internalFormat, int width, DataRangeT&& data, int mipmapLevel = 0)
        {
            ...
            if (int(data_span.size()) != width)
            {
                error() << "Texture size is " << width << " = " << width << " but " << data_span.size()
        ...
        template <typename DataRangeT>
        void setSubData(int x, int width, DataRangeT&& data, int mipmapLevel = 0)
        {
            static_assert(cc::is_any_contiguous_range<DataRangeT>, "data must be a contiguous range!");
            auto data_span = cc::span(data);
            using DataT = std::decay_t<decltype(data_span[0])>;
            ...
            setSubData(x, width, glTypeOf<DataT>::format, glTypeOf<DataT>::type, data_span.data(), mipmapLevel);
        }
        /// Same as above
        /// Usage:
        ///   tg::vec3 texData[][] = { ... }
...
public:
    ...
    Texture1D(Shape const& shape);

    /// Binds this texture.
    /// Unbinding is done when the returned object runs out of scope.
    GLOW_NODISCARD BoundTexture1D bind() { return {this}; }
public: // static construction
    ...
    /// Creates a 1D texture from Shape
    static SharedTexture1D create(Shape const& shape);
    ...
    static SharedTexture1D createStorageImmutable(Shape const& shape);
