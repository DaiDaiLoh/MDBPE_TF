...
public:
    ...
    struct BoundTextureCubeMap
    {
        ...
        void setData(GLenum internalFormat, GLenum target, int width, int height, DataRangeT&& data, int mipmapLevel = 0)
        {
        ...
        template <typename DataRangeT>
        void setSubData(GLenum target, int x, int y, int width, int height, DataRangeT&& data, int mipmapLevel = 0)
        {
            static_assert(cc::is_any_contiguous_range<DataRangeT>, "data must be a contiguous range!");
            auto data_span = cc::span(data);
            using DataT = std::decay_t<decltype(data_span[0])>;
            ...
            setSubData(target, x, y, width, height, glTypeOf<DataT>::format, glTypeOf<DataT>::type, data_span.data(), mipmapLevel);
        }
        /// Same as above
        /// Usage:
        ///   tg::vec3 texData[][] = { ... }
...
public:
    ...
    TextureCubeMap(Shape const& shape);

    /// Binds this texture.
    /// Unbinding is done when the returned object runs out of scope.
    GLOW_NODISCARD BoundTextureCubeMap bind() { return {this}; }
public: // static construction
    ...
    /// Creates a CubeMap texture from Shape
    static SharedTextureCubeMap create(Shape const& shape);
    static SharedTextureCubeMap create(SizeT size, GLenum internalFormat = GL_RGBA) { return create(size.width, size.height, internalFormat); }
    ...
    static SharedTextureCubeMap createStorageImmutable(SizeT size, GLenum internalFormat, int mipmapLevels = 0) { return createStorageImmutable(size.width, size.height, internalFormat, mipmapLevels); }
    static SharedTextureCubeMap createStorageImmutable(Shape const& shape);
