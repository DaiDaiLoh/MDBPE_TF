...
public:
    ...
    struct BoundTextureCubeMapArray
    {
        ...
        void setData(GLenum internalFormat, GLenum target, int width, int height, int layers, DataRangeT&& data, int mipmapLevel = 0)
        {
            ...
            if (int(data_span.size()) != width * height * layers)
            {
                error() << "Texture size is " << width << " x " << height << " x " << layers << " = " << width * height * layers << " but " << data_span.size()
        ...
        template <typename DataRangeT>
        void setSubData(GLenum target, int x, int y, int l, int width, int height, int layers, DataRangeT&& data, int mipmapLevel = 0)
        {
            static_assert(cc::is_any_contiguous_range<DataRangeT>, "data must be a contiguous range!");
            auto data_span = cc::span(data);
            using DataT = std::decay_t<decltype(data_span[0])>;
            ...
            setSubData(target, x, y, l, width, height, layers, glTypeOf<DataT>::format, glTypeOf<DataT>::type, data_span.data(), mipmapLevel);
        }
        /// Same as above
        /// Usage:
        ///   tg::vec3 texData[][] = { ... }
